...
namespace ConsoleService
{
    ...
    public class ServiceBase : Component
    {
        ...
        static extern bool StartServiceCtrlDispatcher(IntPtr entry);
        [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
        static extern unsafe bool SetServiceStatus(IntPtr serviceStatusHandle, SERVICE_STATUS* status);

        [DllImport("advapi32.dll", CharSet = CharSet.Unicode, SetLastError = true)]
        static extern IntPtr RegisterServiceCtrlHandlerEx(string serviceName, ServiceControlCallbackEx callback, IntPtr userData);
        ...
        /// <devdoc>
        /// <para>Creates a new instance of the <see cref='ServiceBase'/> class.</para>
        /// </devdoc>
        public ServiceBase()
        {
        ...
        public unsafe void RequestAdditionalTime(int milliseconds)
        {
            fixed (SERVICE_STATUS* pStatus = &_status)
            {
                ...
                {
                    throw new InvalidOperationException("NotInPendingState");
                }
        ...
        public bool CanHandlePowerEvent
        {
            ...
            set
            {
                if (_commandPropsFrozen)
                    throw new InvalidOperationException("CannotChangeProperties");
        ...
        public string ServiceName
        {
            ...
            {
                if (_nameFrozen)
                    throw new InvalidOperationException("CannotChangeName");
        ...
        private void DeferredCustomCommand(int command)
        {
            try
            {
                ...
                WriteLogEntry("CommandSuccessful");
            }
            catch (Exception e)
            {
                WriteLogEntry("CommandFailed"+ e, true);
        ...
        private unsafe void DeferredPause()
        {
            fixed (SERVICE_STATUS* pStatus = &_status)
            {
                ...
                {
                    OnPause();
                    WriteLogEntry("PauseSuccessful");
                    _status.currentState = ServiceControlStatus.STATE_PAUSED;
                }
                catch (Exception e)
                {
                    _status.currentState = ServiceControlStatus.STATE_RUNNING;
                    WriteLogEntry("PauseFailed"+ e, true);
        ...
        private void DeferredPowerEvent(int eventType, IntPtr eventData)
        {
            ...
            try
            {
                var status = (PowerBroadcastStatus)eventType;
                var statusResult = OnPowerEvent(status);

                WriteLogEntry("PowerEventOK");
            }
            catch (Exception e)
            {
                WriteLogEntry("PowerEventFailed" + e, true);
        ...
        private void DeferredSessionChange(int eventType, int sessionId)
        {
            ...
            {
                WriteLogEntry("SessionChangeFailed"+ e, true);
        ...
        private unsafe void DeferredStop()
        {
            fixed (SERVICE_STATUS* pStatus = &_status)
            {
                var previousState = _status.currentState;
                ...
                catch (Exception e)
                {
                    ...
                    SetServiceStatus(_statusHandle, pStatus);
                    WriteLogEntry("StopFailed"+ e, true);
                    throw;
                }
            }
        }
        ...
        private unsafe void DeferredShutdown()
        {
            ...
            {
                WriteLogEntry("ShutdownFailed"+ e, true);
                throw;
            }
        }

        /// <devdoc>
        /// <para>When implemented in a derived class, <see cref='ServiceBase.OnCustomCommand'/>
        ...
        public static void Run(ServiceBase[] services)
        {
            ...
            var sizeOfSERVICE_TABLE_ENTRY = Marshal.SizeOf<SERVICE_TABLE_ENTRY>();

            var entriesPointer = Marshal.AllocHGlobal(checked((services.Length + 1) * sizeOfSERVICE_TABLE_ENTRY));
            try
            {
                var entries = new SERVICE_TABLE_ENTRY[services.Length];
                var multipleServices = services.Length > 1;
                IntPtr structPtr;
                ...
                var lastEntry = new SERVICE_TABLE_ENTRY();
                ...
                var res = StartServiceCtrlDispatcher(entriesPointer);
                ...
                foreach (var service in services)
                {
                    ...
                    if (!res)
                    {
                        service.WriteLogEntry("StartFailed" +errorMessage, true);
                    }
                }
            }
            finally
        ...
        private void Initialize(bool multipleServices)
        {
            if (!_initialized)
            {
                ...
                _commandCallbackEx = ServiceCommandCallbackEx;
                _handleName = Marshal.StringToHGlobalUni(ServiceName);
        ...
        {
            var entry = new SERVICE_TABLE_ENTRY();
        ...
        private void ServiceQueuedMainCallback(object state)
        {
            ...
            {
                WriteLogEntry("StartFailed"+ e, true);
                _status.currentState = ServiceControlStatus.STATE_STOPPED;
        ...
        public unsafe void ServiceMainCallback(int argCount, IntPtr argPointer)
        {
            fixed (SERVICE_STATUS* pStatus = &_status)
            {
                ...
                if (argCount > 0)
                {
                    var argsAsPtr = (char**)argPointer.ToPointer();
                ...
                {
                    var errorMessage = new Win32Exception().Message;
                    WriteLogEntry("StartFailed"+ errorMessage, true);
                }
                ...
                var statusOK = SetServiceStatus(_statusHandle, pStatus);
                ...
                ThreadPool.QueueUserWorkItem(ServiceQueuedMainCallback, args);
                ...
                if (!statusOK)
                {
                    WriteLogEntry("StartFailed"+ new Win32Exception().Message, true);
                    _status.currentState = ServiceControlStatus.STATE_STOPPED;
                    SetServiceStatus(_statusHandle, pStatus);
                }
            }
